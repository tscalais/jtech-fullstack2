...
let useTasksStore: typeof import('../tasks').useTasksStore
...
describe('tasks store', () => {
  beforeEach(async () => {
    ...
    ;({ useTasksStore } = await import('../tasks'))
  })
  ...
  it('renameTask para duplicado lança erro', () => {
    ...
    const id = store.tasks.find((t:any) => t.name==='Uma')!.id
  ...
  it('deleteTask remove e ajusta ativa', () => {
    ...
    store.createTask('A')
    store.createTask('B')
    const idA = store.tasks.find((t:any)=>t.name==='A')!.id
    store.deleteTask(idA)
    expect(store.tasks.some((t:any)=>t.id===idA)).toBe(false)
    expect(store.activeTaskId).toBe(store.tasks[0].id)
  })
  ...
  it('setActiveTask muda id', () => {
    ...
    store.createTask('A')
    store.createTask('B')
    const idB = store.tasks.find((t:any)=>t.name==='B')!.id
